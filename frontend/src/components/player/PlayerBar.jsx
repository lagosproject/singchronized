  ...
  return (
    <div className="bottom-player-bar">
      <div style={{ display: 'flex', flexDirection: 'column', width: '200px', flexShrink: 0 }}>
        <span style={{ fontSize: '0.75rem', color: 'var(--primary)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{t("singingNow")}</span>
      ...
      <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexShrink: 0 }}>
        {status.is_paused ? (
          <button onClick={resume} className="interactive-btn" style={{ padding: '8px 12px', borderRadius: '8px' }} title={t("resumeSpace")}><PlayCircle size={14} /></button>
        ) : (
          <button onClick={pause} className="interactive-btn secondary-btn" style={{ padding: '8px 12px', borderRadius: '8px' }} title={t("pauseSpace")}><Pause size={14} /></button>
        )}
      ...
      <button
        ...
        className="interactive-btn secondary-btn"
        style={{ padding: '8px 12px', borderRadius: '8px' }}
        title={t("fullscreenLyricsF")}
      >
        <Maximize2 size={14} /> {t("fullscreen")}
      </button>
    </div>
  );
}
